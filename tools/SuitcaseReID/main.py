...
class SuitcaseReIDRunner(BaseRunner):
    def update_labels(self):
        ...
        # generate pseudo labels through clustering
        pseudo_labels, label_centers = self.label_generator(
            self._epoch, memory_features=memory_features, print_freq=self.print_freq
        )

        # Print clustering statistics for single-camera suitcase data
        if pseudo_labels and len(pseudo_labels) > 0:
            cluster_info = pseudo_labels[0]  # First dataset's labels
            unique_labels = set(cluster_info)
            if -1 in unique_labels:
                unique_labels.remove(-1)
            num_clusters = len(unique_labels)
            num_outliers = list(cluster_info).count(-1)
            cluster_sizes = {}
            for label in unique_labels:
                cluster_sizes[label] = list(cluster_info).count(label)
            
            print(f"Suitcase clustering stats: {num_clusters} clusters, {num_outliers} outliers")
            print(f"Cluster size distribution: min={min(cluster_sizes.values()) if cluster_sizes else 0}, max={max(cluster_sizes.values()) if cluster_sizes else 0}")
            
            # If we have a clustering issue (too few clusters)
            if num_clusters < 10:
                print(f"Warning: Only {num_clusters} clusters found. Training may be unstable.")
                print("Proceeding with small number of clusters, but results may not be optimal.")
                
                # Ensure we have at least some diversity
                if num_clusters <= 1:
                    # Split the single cluster into artificial sub-clusters
                    print("Splitting the single cluster into artificial sub-clusters for training stability")
                    non_outlier_indices = [i for i, label in enumerate(cluster_info) if label != -1]
                    num_points = len(non_outlier_indices)
                    
                    # Create at least 10 sub-clusters by splitting the single one
                    num_subclusters = max(10, num_clusters)
                    points_per_cluster = num_points // num_subclusters
                    
                    # Assign new subclusters
                    for i, idx in enumerate(non_outlier_indices):
                        new_label = i // points_per_cluster
                        if new_label < num_subclusters:  # Ensure we don't exceed the target number
                            cluster_info[idx] = new_label
                    
                    # Update the pseudo_labels
                    pseudo_labels[0] = cluster_info
                    print(f"Created {num_subclusters} artificial subclusters")
    ...
    def train_step(self, iter, batch):
        # Try-except block to catch and handle StopIteration errors
        try:
            ...
            print("Caught StopIteration in train_step. Creating new data iterator.")
            return None  # Signal to the runner to skip this step


def parge_config():
    parser = argparse.ArgumentParser(description="SuitcaseReID training with SpCL for single-camera unlabeled data")
...
def main():
    ...
    print("Training on single-camera unlabeled suitcase dataset")
    ...
    print("Extracting features for all suitcase images...")
    test_features = {}
    
    for i, (loader, query, gallery) in enumerate(zip(test_loaders, queries, galleries)):
        # Extract query features
        q_features = extract_features(model, loader, query, with_path=True, prefix="Query: ")
        test_features["query"] = q_features
        
        # Extract gallery features
        g_features = extract_features(model, loader, gallery, with_path=True, prefix="Gallery: ")
        test_features["gallery"] = g_features
    
    # Save features to disk
    feature_path = cfg.work_dir / "suitcase_features.pth"
    torch.save(test_features, feature_path)
