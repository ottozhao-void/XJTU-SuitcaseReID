...
__all__ = ["ReIDBaseModel", "ReIDModelWithMask", "TeacherStudentNetwork",
            "build_model", "build_model_with_mask", "build_gan_model"]
...
class ReIDModelWithMask(nn.Module):
    """
    Extended ReID model with mask generation capability.
    Contains:
    + one backbone with mask decoder, e.g. ResNet101WithMask
    + one global pooling layer, e.g. avg pooling
    + one embedding block, (linear, bn, relu, dropout) or (bn, dropout)
    + one classifier
    + mask generation capability
    """
    ...
    def __init__(
        ...
        super(ReIDModelWithMask, self).__init__()

        self.backbone = build_bakcbone(arch, pretrained=pretrained, mask_output_size=mask_output_size)
        
        # Check if backbone has mask generation capability
        self.has_mask = hasattr(self.backbone, 'mask_decoder')
    ...
    def forward(self, x):
        ...
        if not self.training:
            return feat if not self.has_mask else (feat, results.get("mask", None))
...
def build_model_with_mask(
    cfg, num_classes, init=None,
):
    """
    Build a (cross-domain) re-ID model with mask generation capability
    with domain-specfic BNs (optional)
    """

    # construct a reid model with mask
    model = ReIDModelWithMask(
        cfg.MODEL.backbone,
        num_classes,
        cfg.MODEL.pooling,
        cfg.MODEL.embed_feat,
        cfg.MODEL.dropout,
        pretrained=cfg.MODEL.imagenet_pretrained,
        mask_output_size=(cfg.DATA.height, cfg.DATA.width),
    )
